...
func TestRuleV02UsesCanonicalDimensionsAndActionIDs(t *testing.T) {
	t.Parallel()

	schema := readObject(t, "sprach-a-lyzer_rule_v0.2.json")
	...
	actionTypes := []string{}
	for _, definitionName := range []string{"addContribution", "factorAction", "valueAction", "dimensionOnlyAction", "keyAction", "terminalAction"} {
	...
	if bytes.Contains(data, []byte("FREE_WILL")) {
		t.Error("rule v0.2 schema contains legacy dimension ID")
	}
}
...
func TestCanonicalRuleFixtureUsesOnlyRegisteredActions(t *testing.T) {
	...
	fixture := readObject(t, "../../data/seed/sprach-a-lyzer_rule-contract-fixture_v0.2.json")
	if fixture["contract_version"] != "0.2" {
		t.Fatalf("fixture contract version = %#v; want 0.2", fixture["contract_version"])
	}
...
func TestPolicyRegistrySchemaAndSeedAreJSON(t *testing.T) {
	t.Parallel()
	readObject(t, "sprach-a-lyzer_policy-registry_v0.1.json")
	readObject(t, "sprach-a-lyzer_parameter_v0.1.json")
	readObject(t, "../../data/seed/sprach-a-lyzer_policy-registry_v0.1.json")
