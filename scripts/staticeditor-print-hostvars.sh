...
usage() {
  cat <<'USAGE'
Usage: ./scripts/staticeditor-print-hostvars.sh <domain> [allowed-email1 allowed-email2 ...] [--login-domain=<domain>] [--repo-root=<path>] [--static-root=<path>] [--undo-backups=<path>] [--backup-root=<path>] [--cookie-secret=<secret>]
...
BACKUP_ROOT="${BACKUP_ROOT:-/srv/editor-undo-archive/${DOMAIN}}"
...
echo "static_editor_undo_backups: \"${BACKUP_ROOT}\""
echo "static_editor_repo_root: \"${REPO_ROOT}\""
echo "static_editor_from_email: \"\""
...
echo "static_editor_main_selector: \"main, article, .content, body\""
echo "static_editor_allowed_block_tags: \"div,section,article,blockquote,h1,h2,h3,h4,h5,h6,p,ul,ol,li\""
echo "static_editor_allowed_inline_tags: \"strong,em,a,span,button,br\""
